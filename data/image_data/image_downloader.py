...
def getpic(x1, y1, x2, y2, z, source='google', outdir="MAP_OUT.png", style='s'):
    '''
    ...
    for i, data in enumerate(datas):
        ...
        y, x = i // lenx, i % lenx
        tile_x, tile_y = pos1x + x, pos1y + y
        filename = f"{z}_{tile_x}_{tile_y}_s.png"
        filepath = os.path.join(outdir, filename)
        small_pic.save(filepath)

    print(f'Pics saved to directory {outdir}！')
...
if __name__ == '__main__':
    # 获取(左上角精度，左上角维度，右下角精度，右下角维度）之间的1km*1km的卫星图，zoom = 16
    x = getpic(116.030000, 40.150000, 116.790000, 39.750000,
               16, source='amap', style='s', outdir="data/images/trash")
    # x = {"LT": (float(53936), float(24800)), "RT": (float(53937), float(24800)), "LB": (float(53936), float(24801)), "RB": (float(53937), float(24801)), "z": 16}
    # 获取某个地点的瓦片坐标对应的经纬度坐标，不同坐标系可通过第三个参数设置
    # file_out(x, "zb18.txt", "wgs")
